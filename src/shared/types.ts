...
export interface IPCApi {
  ...
  // Orders
  orders: {
    getAll: (filters?: { startDate?: string; endDate?: string; userId?: number; customerId?: number }) => Promise<OrderWithItems[]>;
  ...
  templates: {
    ...
    uploadLogo: (templateId: number) => Promise<InvoiceAsset | null>;
    addQRCode: (templateId: number, qrData: {
      label: string;
      data: string;
      errorCorrectionLevel?: 'L' | 'M' | 'Q' | 'H';
      size?: number;
    ...
    updateAssetMeta: (assetId: number, metaJson: string) => Promise<void>;
  };
  ...
  installments: {
    ...
    getOverdue: () => Promise<Array<Installment & { plan: InstallmentPlan; order: Order }>>;
  ...
  // Payments
  payments: {
    getAll: (filters?: { startDate?: string; endDate?: string }) => Promise<Payment[]>;
    getByOrder: (orderId: number) => Promise<Payment[]>;
    recordInstallmentPayment: (installmentId: number, amount: number, method: string) => Promise<void>;
  };
  ...
  // Print
  print: {
    generatePDF: (orderId: number, options?: {
      outputPath?: string;
      billSize?: string;
      layout?: string;
    }) => Promise<string>;
